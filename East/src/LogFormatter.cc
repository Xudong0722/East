...
class MessageFormatItem : public LogFormatter::FormatItem {
 public:
  ...
  void format(std::ostream& os, std::shared_ptr<Logger> logger,
              LogLevel::Level level, LogEvent::sptr event) override {
    os << event->getContent();
  }
};
...
class NormalStringFormatItem : public LogFormatter::FormatItem {
 public:
  NormalStringFormatItem(const std::string& __ = "") : m_normal_str(__) {}
  void format(std::ostream& os, std::shared_ptr<Logger> logger,
              LogLevel::Level level, LogEvent::sptr event) override {
    os << m_normal_str;
  }
...
std::string LogFormatter::format(Logger::sptr logger, LogLevel::Level level,
                                 LogEvent::sptr event) {
...
void LogFormatter::init() {
  std::vector<std::tuple<std::string, std::string, int>>
      vec_pattern{};  //{pattern, format, type}
  ...
  for (auto i = 0u; i < m_pattern.size(); ++i) {
    ...
    while (j < m_pattern.size()) {
      if (!fmt_status && !isalpha(m_pattern[j]) && m_pattern[j] != '{' &&
          m_pattern[j] != '}') {
  ...
  for (const auto& item : vec_pattern) {
    if (std::get<2>(item) == 0) {
      m_items.emplace_back(
          std::make_shared<NormalStringFormatItem>(std::get<0>(item)));
    } else {
      ...
      if (cit == s_format_items.end()) {
        m_items.emplace_back(std::make_shared<NormalStringFormatItem>(
