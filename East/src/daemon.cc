...
static East::ConfigVar<uint32_t>::sptr g_daemon_restart_interval =
    East::Config::Lookup("daemon.restart_interval", (uint32_t)5,
...
static int real_daemon(int argc, char** argv,
  ...
  ProcessInfoMgr::GetInst()->parent_start_time =
  ...
  while (true) {
    ...
    if (pid == 0) {
      ...
      ProcessInfoMgr::GetInst()->main_start_time =
      ...
      //错误处理
      ELOG_ERROR(g_logger) << "fork failed, result: " << pid
                           << " errno: " << errno
