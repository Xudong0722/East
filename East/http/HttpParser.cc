...
static East::ConfigVar<uint64_t>::sptr g_http_req_parser_buffer_size =
    East::Config::Lookup<uint64_t>("http.req_parser.buffer_size", 4 * 1024ull,
                                   "http request parser buffer size");
static East::ConfigVar<uint64_t>::sptr g_http_req_max_body_size =
    East::Config::Lookup<uint64_t>("http.req_parser.max_body_size",
                                   6 * 1024 * 1024ull,
                                   "http request parser max body size");
static East::ConfigVar<uint64_t>::sptr g_http_resp_parser_buffer_size =
    East::Config::Lookup<uint64_t>("http.resp_parser.buffer_size", 4 * 1024ull,
                                   "http response parser buffer size");
static East::ConfigVar<uint64_t>::sptr g_http_resp_max_body_size =
    East::Config::Lookup<uint64_t>("http.resp_parser.max_body_size",
                                   6 * 1024 * 1024ull,
                                   "http response parser max body size");
...
struct _RequestSizeIniter {
  _RequestSizeIniter() {
    ...
    g_http_req_parser_buffer_size->addListener(
        [](const uint64_t& old_v, const uint64_t new_v) {
          s_http_req_parser_buffer_size = new_v;
          ELOG_INFO(g_logger) << "http.req_parser.buffer_size changed from "
                              << old_v << " to " << new_v;
        });
    g_http_req_max_body_size->addListener(
        [](const uint64_t& old_v, const uint64_t new_v) {
          s_http_req_max_body_size = new_v;
          ELOG_INFO(g_logger) << "http.req_parser.max_body_size changed from "
                              << old_v << " to " << new_v;
        });
    g_http_resp_parser_buffer_size->addListener(
        [](const uint64_t& old_v, const uint64_t new_v) {
          s_http_resp_parser_buffer_size = new_v;
          ELOG_INFO(g_logger) << "http.resp_parser.buffer_size changed from "
                              << old_v << " to " << new_v;
        });
    g_http_resp_max_body_size->addListener(
        [](const uint64_t& old_v, const uint64_t new_v) {
          s_http_resp_max_body_size = new_v;
          ELOG_INFO(g_logger) << "http.resp_parser.max_body_size changed from "
                              << old_v << " to " << new_v;
        });
  }
};
