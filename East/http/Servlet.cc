...
int32_t FunctionServlet::handle(HttpReq::sptr req, HttpResp::sptr rsp,
                                HttpSession::sptr session) {
...
int32_t ServletDispatch::handle(HttpReq::sptr req, HttpResp::sptr rsp,
                                HttpSession::sptr session) {
...
void ServletDispatch::addGlobServlet(const std::string& uri,
  ...
  MutexType::WLockGuard wlock(m_mutex);
  auto it =
      std::find_if(m_globs.begin(), m_globs.end(),
                   [&uri](const auto& item) { return item.first == uri; });
  if (it != m_globs.end())
    m_globs.erase(it);
...
Servlet::sptr ServletDispatch::getGlobServlet(const std::string& uri) {
  MutexType::RLockGuard rlock(m_mutex);
  auto it =
      std::find_if(m_globs.begin(), m_globs.end(), [&uri](const auto& item) {
...
int32_t NotFoundServlet::handle(HttpReq::sptr req, HttpResp::sptr rsp,
                                HttpSession::sptr session) {
