...
East::ConfigVar<std::unordered_map<std::string, int>>::sptr g_str_int_umap_value_config =
    East::Config::Lookup("system.str_int_umap", std::unordered_map<std::string, int>{{"k", 444}}, "system str int umap");
...
East::ConfigVar<Person>::sptr g_person_value_config 
    = East::Config::Lookup("system.class.person", Person{"elvis", 25, true}, "system class person");
...
#define for_loop_map(g_val, name, prefix)                                                      \
    ...
    ELOG_INFO(ELOG_ROOT()) << "Before:" << g_person_value_config->getValue().toString() << " - " << g_person_value_config->toString();
    g_person_value_config->addListener(101, [](const Person& old_person, const Person& new_person){
        ELOG_INFO(ELOG_ROOT()) << "Old value: " << old_person.toString() <<", new value: " << new_person.toString();
    });
    ...
    ELOG_INFO(ELOG_ROOT()) << "After:" << g_person_value_config->getValue().toString() << " - " << g_person_value_config->toString();
